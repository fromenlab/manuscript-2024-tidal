...
```{r}
parent <- "deposition/20231127-full-lung/"
```
...
d <- make_path("data/raw/20231127_full_lung_CF_analysis.xlsx") %>% read_excel(range = "Analysis!AQ18:AX24")
```
...
```{r}
mt_direct <- make_path("data/raw/20231127_full_lung_CF_analysis.xlsx") %>% read_excel(range = "Analysis!AH4", col_names = F)
tb_direct <- make_path("data/raw/20231127_full_lung_CF_analysis.xlsx") %>% read_excel(range = "Analysis!AH5", col_names = F)
```
...
```{r}
lobe_deposition_mass %>% write_csv(make_path("data/summary-full-lung-20231127.csv"))
```

### Get Emitted Dose, Deposited Dose

```{r}
xlfile <- make_path("data/raw/20231127_full_lung_CF_analysis.xlsx")

emitted_dose <- xlfile %>% 
  read_excel(range = "Analysis!AK7", col_names = F) %>% 
  .$...1

deposited_dose <- xlfile %>% 
  read_excel(range = "Analysis!AK10", col_names = F) %>% 
  .$...1

lobe_dose <- xlfile %>% 
  read_excel(range = "Analysis!AK14", col_names = F) %>% 
  .$...1
