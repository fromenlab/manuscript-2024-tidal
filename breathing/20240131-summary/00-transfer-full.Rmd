---
title: "Figure 4 - Breathing Summary"
author: "irw"
...
full_lung_breaths %>% 
  group_by(name, peak) %>% 
  summarize(breaths_per_minute = 1/(sum(time_s)/60)) %>% 
  summarize(bpm = mean(breaths_per_minute), 
            sd = sd(breaths_per_minute)) %>% 
  mutate(maneuver = case_match(
    name,
    "d500-s250-home" ~ "Fast-tidal",
    "d500-s450-full-exhale" ~ "Fast-moderate",
    "d2100-s250-home" ~ "Resting-tidal",
    "d2100-s450-full-exhale" ~ "Resting-moderate"
  )
  )
...
# Resting
t.test(
  full_lung_breaths %>% subset(maneuver == "i" & name == "d2100-s250-home") %>% .$avg_q,
  full_lung_breaths %>% subset(maneuver == "i" & name == "d2100-s450-full-exhale") %>% .$avg_q
  )

# Fast
t.test(
  full_lung_breaths %>% subset(maneuver == "i" & name == "d500-s250-home") %>% .$avg_q,
  full_lung_breaths %>% subset(maneuver == "i" & name == "d500-s450-full-exhale") %>% .$avg_q
  )
```
...
format_full_lung_seconds <- function(plot) {
  plot +
    coord_cartesian(
      xlim = c(0, 0.7), 
      ylim = c(-50, 50),
    ...
    scale_x_continuous(breaks = c(0, 10, 20, 30, 40)/60, labels = c(0, 10, 20, 30, 40)) +
    labs(x = "Time (seconds)") +
    theme_bw(base_size = 8) +
    theme(legend.position = "none")
}
...
# Set which of the above chunks to evaluate, depending on desired axis units
...
knitr::opts_chunk$set(fig.width = 174/3*2/25.4, fig.height = 40*2/25.4)
